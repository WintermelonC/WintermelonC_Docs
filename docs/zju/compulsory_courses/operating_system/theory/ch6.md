...
##### 7.1 Solaris Synchronization

Solaris 操作系统的同步机制旨在支持复杂的现代计算环境，包括多任务（同时运行多个程序）、多线程（程序内多个执行流，包含有严格时限的实时线程）和多处理（在多个 CPU 核心上运行）

使用 adaptive mutexes（自适应互斥锁）：这是用于保护短代码段（临界区）的主要锁类型。自适应意味着它会根据实际情况在自旋（忙等待）和阻塞（让出 CPU）之间做出高效选择，以减少上下文切换的开销

使用 condition variables 和 readers-writers locks：当线程需要持有锁较长时间（较长代码段）时，会使用这些机制。条件变量允许线程在某个条件不满足时主动释放锁并等待，当条件可能满足时再被唤醒，从而避免不必要的忙等待。读写锁允许多个读者线程同时访问数据，但写者线程独占访问，这适用于读多写少的场景，能提高并发性

使用 turnstile（转门）：转门是一个核心的数据结构，用于管理所有等待同一个锁的线程队列。它的关键作用是对等待线程进行排序，这有助于防止某些线程无限期地等待（饥饿），并确保公平性或满足优先级调度（尤其是对实时线程至关重要）。当一个锁被释放时，转门机制决定下一个哪个线程可以获得锁

##### 7.2 Windows XP Synchronization

Windows XP 操作系统内核中采用的同步机制，其策略根据硬件架构有所不同

1. 单处理器系统：使用 interrupt masks（中断屏蔽）。这是一种轻量级技术，通过临时禁止 CPU 响应中断，来保证当前正在执行的内核代码段（临界区）不会被中断处理程序抢占，从而实现对全局资源的独占访问
2. 多处理器系统：使用 spinlock（自旋锁）。因为在多 CPU 环境下，禁用单个 CPU 的中断无法阻止其他 CPU 上的线程同时访问资源。自旋锁通过忙等待（不断循环检查锁是否可用）来保护短期的临界区，避免了线程上下文切换的开销

dispatcher object（分发器对象）是 Windows XP 中一个统一且灵活的同步抽象。一个分发器对象可以根据配置，表现出不同的同步原语的行为：可以充当互斥锁，实现互斥访问；可以充当信号量，控制对一定数量资源的访问。此外，分发器对象也用于实现事件

事件是一种同步对象，其行为与线程同步中的条件变量非常相似。它允许一个线程等待另一个线程完成某项工作或达到某个状态（即事件发生）。当等待的事件发生时，等待的线程会被唤醒。这是实现线程间复杂协调与通信的关键机制

##### 7.3 Linux Synchronization

Linux 操作系统内核中使用两种基本同步方法

1. 禁用中断：这是用于在单处理器系统上保护非常短的临界区的一种简单而有效的方法。通过临时禁止 CPU 响应中断，内核可以确保正在执行的代码序列（临界区）不会被中断处理程序抢占。这防止了可能导致数据不一致的竞态条件。由于这会影响到系统的响应能力和实时性，因此仅适用于执行速度非常快的代码段
2. 提供的同步原语：

    1. 自旋锁：这是在多处理器（SMP）系统上和保护可能被中断处理程序访问的临界区时使用的主要机制。线程会自旋（忙等待）直到锁变得可用。对于短临界区，这避免了上下文切换的开销
    2. 信号量：这是一种更通用的同步机制，当临界区可能执行较长时间时使用。如果锁不可用，线程会进入睡眠状态（阻塞），让出 CPU 给其他任务，适用于可能睡眠的上下文（如进程上下文）

##### 7.4 Pthreads Synchronization

Pthreads API 是独立于操作系统的是其最重要的特点。这意味着使用 Pthreads 同步原语（互斥锁、条件变量）编写的多线程程序，可以在任何符合 POSIX 标准的操作系统（如 Linux、BSD、macOS 等）上编译和运行，而无需修改代码。这为开发者提供了统一的编程接口

标准提供的同步原语：

1. 互斥锁：用于实现基本的互斥访问，保护临界区，确保同一时间只有一个线程可以执行该段代码
2. 条件变量：允许线程在某个条件不成立时主动阻塞，并等待其他线程在改变条件后发出通知。它必须与互斥锁结合使用，是实现复杂线程间协调（如生产者-消费者模型）的关键工具

非可移植的扩展：读写锁和自旋锁是常见的、有用的同步机制，但它们最初并不在基础的 Pthreads 标准中。不同的 UNIX 系统可能会以扩展的形式提供这些锁，但由于它们不是标准强制要求的，因此使用这些扩展的代码可能无法在所有支持 Pthreads 的平台之间直接移植。后来的 POSIX 标准（如 POSIX.1-2001）已经将读写锁纳入其中，成为标准的一部分

## 8 Atomic Transactions

原子事务是数据库系统和需要可靠性的应用中的关键机制，它确保一系列操作要么全部成功执行，要么全部不执行，即使在发生故障时也是如此。这通过满足 ACID 属性（原子性、一致性、隔离性、持久性）来实现

1. log-based recovery：这是最常用的恢复技术。系统维护一个日志，这是一个特殊的记录序列，详细描述了事务所做的修改。日志中会记录事务的开始、对数据的更新（包括更新前的值-旧值，和更新后的值-新值）以及事务的提交或中止。当发生故障后系统重启时，恢复管理器会使用这个日志来确保原子性和持久性：撤销未完成事务（未提交）的所有影响。重做已提交事务的所有影响，这些更改可能尚未完全写入持久存储
2. checkpoint：这是一种优化日志恢复的技术。定期地，系统会执行一个检查点，它将当前内存中的所有修改数据强制写入持久存储，并在日志中记录一个检查点标记。检查点可以大大缩短恢复时间。在恢复时，系统只需要处理最后一个检查点之后开始的日志记录，因为检查点之前提交事务的更改已经安全地存储在磁盘上了
3. concurrent atomic transaction：为了提高性能，数据库系统需要允许多个事务同时运行。这部分涉及并发控制协议。这些协议（如锁协议或时间戳排序）的目的是管理并发事务的交错执行，使得最终结果与这些事务按某种顺序串行执行的结果相同。这确保了隔离性，防止并发事务相互干扰，导致数据不一致