...
假设工作集窗口 $Δ$ = 10,000 时间单位，采样间隔 = 50,00 时间单位，每个页面用 2 位记录，1 位记录前半个窗口是否被访问，1 位记录后半个窗口是否被访问
...
!!! tip "page size"
    ...
    1. 内部碎片：较小的页面大小可以减少内部碎片，提高内存利用率 → 倾向于较小页面
    2. 页表大小：页面越小，进程所需页表的条目就越多，导致页表本身占用更多内存 → 倾向于较大页面
    3. I/O 开销：每次缺页中断需要从磁盘读取一个页面。较大的页面意味着每次 I/O 能传输更多数据，摊薄固定开销，提高效率 → 倾向于较大页面
    4. 局部性原理：较小的页面能更精确地匹配程序的访问局部性，避免将不相关的代码或数据载入内存 → 倾向于较小页面
...
!!! tip "TLB reach"
    ...
    1. 增大页面大小：相同 TLB 条目数可映射更多内存。但可能导致内部碎片增加，因为小内存对象也会占用整个大页面
    2. 支持多种页面大小：允许不同应用程序或内存区域使用不同大小的页面。需要大页面的程序可以使用大页面以提高 TLB 覆盖范围，不需要大页面的程序仍可使用小页面以减少碎片

!!! tip "Program Structure"

    数组 `data` 是一个 128 行 × 128 列的整数矩阵。假设每一行存储在单独的一个页面中（即一个物理内存帧），因此整个数组分布在 128 个不同的页面中

    ```c linenums="1" title="列优先访问"
    for (j = 0; j < 128; j++)
        for (i = 0; i < 128; i++)
            data[i][j] = 0;
    ```
...
1. 工作集最小值：进程保证在内存中至少保留的页面数，防止被过度换出
2. 工作集最大值：进程最多可占用的页面数
...
1. Lotsfree：当系统空闲内存高于此值时，基本不进行页面换出。一旦空闲内存低于此值，系统开始启动换页操作以释放内存
2. Desfree：当空闲内存进一步下降到低于此阈值时，系统会提高换页频率，更积极地回收页面
3. Minfree：当内存极度紧张、空闲内存低于此阈值时，系统会启动进程交换（swapping），即将整个进程换出到磁盘，以快速释放大量内存
...
<figure markdown="span">
    ![Img 33](../../../../img/operating_system/ch9/os_ch9_img33.png){ width="600" }
</figure>