...
拥塞窗口的大小直接影响发送到网络中的数据量。初始时或重传后，TCP 使用 Slow Start（慢启动）机制来逐步探索可用带宽：从较小的窗口开始，每收到一个 ACK，拥塞窗口指数增长（通常每 RTT 翻倍），快速接近可用带宽。一旦检测到拥塞（如丢包），则切换到 Congestion Avoidance（拥塞避免）阶段，转为线性增长（AIMD 中的加性增）。慢启动帮助 TCP 快速达到高效传输状态，同时避免初期过度发送引发拥塞
