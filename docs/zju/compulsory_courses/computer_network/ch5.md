...
3. 广泛应用：是 DNS 根服务器和高层域名服务器的基石技术之一
...
3. IP 协议理论最大值：65515字节（总长度 65,535 减去 20 字节 IP 首部）
...
!!! info "IPv6 不存在的字段"
    ...
    2. checksum：IPv6 无检验和字段。数据完整性检查由其他层负责。传输层（TCP/UDP）有自己的校验和来保护数据和端口号，数据链路层（如以太网的 CRC）也保护了整个帧。在网络层再做一次校验被认为是多余的
