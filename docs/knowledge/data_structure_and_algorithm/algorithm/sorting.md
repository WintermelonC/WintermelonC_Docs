...
选择排序是一种简单直观的排序算法，属于 $O(N^2)$ 级别的 **交换类排序**。它的核心思想非常朴素：**每次从未排序部分选出最小（或最大）的元素，放到已排序部分的末尾**

把数组分成"已排序区间"和"未排序区间"两部分：

1. 初始时，已排序区间为空，未排序区间是整个数组
2. 每轮从未排序区间 **选出最小元素**，与未排序区间的第一个元素交换
3. 交换后，已排序区间扩大一个元素，未排序区间缩小一个元素
4. 重复 $N-1$ 轮后，整个数组有序

以升序为例：

1. 在第 `i` 轮（`i` 从 0 开始），在 `arr[i..n-1]` 中找到最小元素的下标 `minIdx`
2. 交换 `arr[i]` 和 `arr[minIdx]`
3. `i++`，继续下一轮，直到 `i = n-1`
...
void selectionSort(std::vector<int>& arr) {
    ...
    for (int i = 0; i < n - 1; ++i) {
        int minIdx = i;                       // 假设当前第一个元素最小
        for (int j = i + 1; j < n; ++j) {     // 在未排序区间找更小的
            if (arr[j] < arr[minIdx])
                minIdx = j;
        }
        if (minIdx != i)                      // 找到更小的才交换
            std::swap(arr[i], arr[minIdx]);
    }
}
```

### 4.2 优化：二元选择排序（一次找最大和最小）

普通选择排序每轮只找一个极值。优化版 **每轮同时找最小值和最大值**，分别放到两端，轮数减半：
...
void selectionSort2(std::vector<int>& arr) {
    ...
    for (int left = 0, right = n - 1; left < right; ++left, --right) {
        ...
        // 关键修正：若最大值本来在 left 位置，它已被换到了 minIdx
...
!!! warning "易错点：`maxIdx == left` 的修正"

    假设最大值恰好在 `left` 位置。先执行 `swap(arr[left], arr[minIdx])` 后，最大值被移动到了 `minIdx` 位置。此时若不修正 `maxIdx`，后面的 `swap(arr[right], arr[maxIdx])` 会把一个错误元素放到右端。所以必须先判断 `if (maxIdx == left) maxIdx = minIdx;`。

### 4.3 复杂度分析

| 项目 | 复杂度 | 说明 |
|---|---|---|
| 最好情况 | $O(N^2)$ | 无论输入如何，比较次数固定 |
| 平均情况 | $O(N^2)$ | 同下 |
| 最坏情况 | $O(N^2)$ | 每轮都要扫描整个未排序区间 |
| 比较次数 | $\frac{N(N-1)}{2}$ | 与输入无关，恒为这个值 |
| 交换次数 | $O(N)$（最多 $N-1$ 次） | 远少于冒泡排序 |
| 空间复杂度 | $O(1)$ | 原地排序 |
| 稳定性 | **不稳定** | 交换会破坏相同元素的相对顺序 |

选择排序最大的特点：**交换次数少**（最多 $N-1$ 次），而比较次数固定为 $\frac{N(N-1)}{2}$。因此当"交换（写入）代价很高、比较代价很低"时，选择排序有独特价值

!!! question "为什么不稳定"

    选择排序的"远距离交换"会打乱相同元素的相对顺序。以 `[2a, 2b, 1]` 为例：
    
    | 步骤 | 数组 | 说明 |
    |---|---|---|
    | 初始 | `[2a, 2b, 1]` | 两个 2，a 在前 b 在后 |
    | 第 1 轮 | `[1, 2b, 2a]` | 找到最小值 1（下标 2），与 `arr[0]`（即 2a）交换 |
    
    交换后 `2a` 被挪到了数组末尾，相对顺序从 `2a → 2b` 变成 `2b → 2a`，因此 **不稳定**
...
| 最好时间 | $O(N^2)$ | $O(N)$（加标志位） | $O(N)$（近乎有序） |
| 比较次数 | 固定 $\frac{N(N-1)}{2}$ | 最多 $\frac{N(N-1)}{2}$ | 最多 $\frac{N(N-1)}{2}$ |
| 交换/移动次数 | **$O(N)$（最少）** | $O(N^2)$ | $O(N^2)$ |
| 稳定性 | 不稳定 | 稳定 | 稳定 |
| 适用场景 | 写入代价高的场景 | 教学、小数据 | 近乎有序的数据 |

- **vs 冒泡**：两者比较次数相同，但选择排序交换次数远少（$O(N)$ vs $O(N^2)$）
- **vs 插入**：插入排序对近乎有序数据可到 $O(N)$ 且稳定，但元素移动次数多；选择排序交换少但永远 $O(N^2)$

### 4.5 与堆排序的联系

堆排序可以看作 **选择排序的优化版**：

> 选择排序每轮用 $O(N)$ 线性扫描找最小值；堆排序用 **堆** 在 $O(\log N)$ 时间内找到最值

两者本质都是"**每轮选出最值放到末尾**"，只是找最值的手段不同：

| 算法 | 找最值的方式 | 单次找最值 | 总时间 |
|---|---|---|---|
| 选择排序 | 线性扫描 | $O(N)$ | $O(N^2)$ |
| 堆排序 | 大顶堆/小顶堆 | $O(\log N)$ | $O(N\log N)$ |
